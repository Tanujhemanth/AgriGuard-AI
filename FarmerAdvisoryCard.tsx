...
export const FarmerAdvisoryCard: React.FC<FarmerAdvisoryCardProps> = ({ advisory, onOpenPrintModal }) => {
  ...
  return (
    <div className="glass-card p-5 lg:p-7 space-y-6 relative border-2 border-emerald-500/30 shadow-2xl">
      ...
      <div className={`p-4 rounded-xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${banner.bg}`}>
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-emerald-950/80 border border-emerald-500/30">
            {banner.icon}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-black tracking-wider uppercase shadow ${banner.badgeBg}`}>
                {advisory.action_status}
              </span>
              <span className="text-xs text-emerald-200/80 font-mono">Deterministic Decision Engine</span>
            </div>
            <h2 className="text-lg font-extrabold text-white mt-1">{banner.title}</h2>
        ...
        <div className="flex items-center gap-2 self-end sm:self-center">
          <button
            ...
            className="px-3 py-2 bg-emerald-900/80 hover:bg-emerald-800 border border-emerald-500/40 text-emerald-200 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-4 rounded-xl bg-emerald-950/50 border border-emerald-500/20 space-y-3">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-emerald-300/70">Target Crop:</span>
              <span className="font-semibold text-white">{advisory.whatsWrong.crop}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-emerald-300/70">Identified Disease/Condition:</span>
              <span className="font-bold text-amber-300">{advisory.whatsWrong.condition}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-emerald-300/70">Pathogen Classification:</span>
              <span className="font-semibold text-emerald-200">{advisory.whatsWrong.pathogenType}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-emerald-300/70">AI Diagnostic Confidence:</span>
              <span className="font-extrabold text-amber-400">{advisory.whatsWrong.confidence}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-emerald-950/50 border border-emerald-500/20 space-y-3">
          ...
          <div className="space-y-2 text-xs">
            <div>
              ...
              <p className="text-xs font-bold text-white bg-emerald-900/40 p-2 rounded-lg border border-emerald-500/20 mt-1">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {advisory.timing_guidance.weatherFactors.map((wf: string, idx: number) => (
                  <span key={idx} className="px-2 py-0.5 rounded bg-emerald-900/80 text-[11px] text-emerald-200 border border-emerald-500/30">
                    • {wf}
                  </span>
                ))}
              </div>
        ...
        <div className="lg:col-span-2 p-5 rounded-xl bg-emerald-950/60 border border-emerald-500/30 space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-emerald-500/20 pb-3">
            <div>
              ...
              <p className="text-xs text-emerald-300/80 mt-0.5">{advisory.treatment_guidance.summary}</p>
            </div>

            {/* Treatment Protocol Tabs */}
            <div className="flex items-center gap-1 bg-emerald-900/80 p-1 rounded-lg border border-emerald-500/30">
              <button
            ...
            <div className="space-y-2">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {advisory.treatment_guidance.organic.map((act: string, i: number) => (
                  <li key={i} className="text-xs text-emerald-100 bg-emerald-900/40 p-2.5 rounded-lg border border-emerald-500/20 flex items-start gap-2">
            ...
            <div className="space-y-2">
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {advisory.treatment_guidance.chemical.map((act: string, i: number) => (
                  <li key={i} className="text-xs text-amber-100 bg-amber-950/40 p-2.5 rounded-lg border border-amber-500/30 flex items-start gap-2">
          ...
          <div className="pt-2 border-t border-emerald-500/20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
              {advisory.treatment_guidance.ppe.map((ppe: string, i: number) => (
                <div key={i} className="p-2 rounded bg-emerald-950/80 border border-emerald-500/20 text-[11px] text-emerald-300">
                  • {ppe}
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-2 p-5 rounded-xl bg-emerald-950/40 border border-emerald-500/20 space-y-3">
          <h3 className="text-sm font-bold text-amber-400 uppercase tracking-wide flex items-center gap-1.5">
          ...
          <div className="space-y-2">
            <ol className="space-y-2 text-xs text-emerald-200">
              {advisory.reasons.map((step: string, i: number) => (
                <li key={i} className="flex items-start gap-2.5 p-2 rounded bg-emerald-900/30 border border-emerald-500/20">
            ...
            <div className="p-3 rounded-lg bg-amber-950/40 border border-amber-500/30 text-xs text-amber-200 mt-2">
