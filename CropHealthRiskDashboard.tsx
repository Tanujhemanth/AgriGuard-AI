...
import { Activity, ShieldCheck, AlertOctagon, Bug, CloudLightning, Droplet, CheckCircle2 } from 'lucide-react';
...
export const CropHealthRiskDashboard: React.FC<CropHealthRiskDashboardProps> = ({
  ...
  const getRiskBadge = (level: string) => {
    switch (level) {
      case 'CRITICAL':
        return 'bg-rose-500/25 text-rose-300 border-rose-500/50 font-black tracking-wider';
      case 'HIGH':
        return 'bg-amber-500/25 text-amber-300 border-amber-500/50 font-extrabold';
      case 'MODERATE':
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-5 glass-card p-5 lg:p-6 space-y-5 border-2 border-emerald-500/30 shadow-2xl">
        <div className="flex items-center justify-between border-b border-emerald-500/20 pb-3">
          <h3 className="text-sm font-extrabold text-emerald-300 uppercase tracking-wide flex items-center gap-1.5">
            <Activity className="w-4 h-4 text-emerald-400" /> AI Crop Health Index
          </h3>
          <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-950 text-amber-300 border border-emerald-500/30">
            PERFECTED INDEX
          </span>
        </div>

        {/* Big Health Score Gauge */}
        <div className="flex items-center justify-between p-4 rounded-xl bg-emerald-950/70 border border-emerald-500/30 shadow-inner">
          <div>
            <span className="text-xs text-emerald-300/80 uppercase font-medium">Foliar Health Score</span>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-4xl font-black text-white tracking-tight">{analysis.cropHealthScore}</span>
          ...
          {/* Meter Badge */}
          <div className="w-18 h-18 rounded-full border-4 border-emerald-500/20 flex flex-col items-center justify-center bg-emerald-950 relative shadow-lg">
            <span className="text-lg font-black text-amber-300">{analysis.cropHealthScore}%</span>
            <span className="text-[9px] text-emerald-400/80 font-mono font-bold">HEALTH</span>
          </div>
        </div>

        {/* Health Stress Breakdown */}
        <div className="space-y-3 text-xs">
          <span className="text-emerald-300/90 font-bold block uppercase tracking-wider text-[11px]">
          ...
          <div>
            <div className="flex justify-between text-[11px] mb-1">
              ...
              <span className="text-rose-400 font-extrabold">-{analysis.healthBreakdown.diseaseImpact} pts</span>
            </div>
            <div className="w-full h-2 bg-emerald-950 rounded-full overflow-hidden border border-emerald-500/20">
          ...
          <div>
            <div className="flex justify-between text-[11px] mb-1">
              ...
              <span className="text-amber-400 font-extrabold">-{analysis.healthBreakdown.visibleDamage} pts</span>
            </div>
            <div className="w-full h-2 bg-emerald-950 rounded-full overflow-hidden border border-emerald-500/20">
          ...
          <div>
            <div className="flex justify-between text-[11px] mb-1">
              ...
              <span className="text-emerald-300 font-extrabold">-{analysis.healthBreakdown.environmentalStress} pts</span>
            </div>
            <div className="w-full h-2 bg-emerald-950 rounded-full overflow-hidden border border-emerald-500/20">
      ...
      <div className="lg:col-span-7 glass-card p-5 lg:p-6 space-y-4 border-2 border-emerald-500/30 shadow-2xl">
        <div className="flex items-center justify-between border-b border-emerald-500/20 pb-3">
          <h3 className="text-sm font-extrabold text-amber-400 uppercase tracking-wide flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {/* Disease Risk */}
          <div className="p-3.5 rounded-xl bg-emerald-950/70 border border-emerald-500/30 space-y-1">
            <div className="flex items-center justify-between text-xs">
              <span className="text-emerald-300 font-bold flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-amber-400" /> Disease Risk
              </span>
              <span className={`px-2 py-0.5 rounded text-[10px] border ${getRiskBadge(risk.diseaseRisk)}`}>
                {risk.diseaseRisk}
              </span>
            </div>
          ...
          <div className="p-3.5 rounded-xl bg-emerald-950/70 border border-emerald-500/30 space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] border ${getRiskBadge(risk.pestRisk)}`}>
                {risk.pestRisk}
              </span>
            </div>
          ...
          <div className="p-3.5 rounded-xl bg-emerald-950/70 border border-emerald-500/30 space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] border ${getRiskBadge(risk.climateRisk)}`}>
                {risk.climateRisk}
              </span>
            </div>
          ...
          <div className="p-3.5 rounded-xl bg-emerald-950/70 border border-emerald-500/30 space-y-1">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] border ${getRiskBadge(risk.waterStressRisk)}`}>
        ...
        <div className="p-3.5 rounded-xl bg-amber-950/50 border border-amber-500/40 text-xs text-amber-200">
